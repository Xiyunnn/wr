...
if check_ip_and_get_info "$IP_VPS"; then
    ...
    echo -e "➥ Hubungi admin ${CYAN}「 ✦ @WahidRomdoni ✦ 」${NC}"
    exit 1
fi
...
show_footer() {
    ...
    echo -e " ${COLOR1}║                  ${WH}• YIWEI VPN STORE •                ${COLOR1}║${NC}"
    echo -e " ${COLOR1}╚════════════════════════════════════════════════════╝${NC}"
}
...
function domain() {
    ...
    install_slowdns() {
        local script_url="https://raw.githubusercontent.com/Xiyunnn/wr/refs/heads/main/SLOWDNS/installsl.sh"
...
10 |10) clear ; wget -q https://github.com/Xiyunnn/wr/raw/refs/heads/main/install-ulang-vps && bash install-ulang-vps ;; 
00 |0) clear ; menu ;; 
